...
module.exports = {
    data: new Discord.SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option.setName('draws')
                .setDescription('Quantidade de empates atuais')
                .setRequired(true)
        )        
        .addIntegerOption(option =>
    ...
    async execute(interaction) {
        try {
            ...
            const draws = interaction.options.getInteger('draws');
            ...
            const insertStmt = Database.prepare('INSERT INTO cbp (guildId, userId, playerName, points, wins, draws, bonus) VALUES (?, ?, ?, ?, ?, ?, ?)');
            const updateStmt = Database.prepare('UPDATE cbp SET points = ?, wins = ?, draws = ?, bonus = ? WHERE guildId = ? AND userId = ?');
            ...
            if (existingData) {
                try {
                    await updateStmt.run(points, wins, draws, bonus, guildId, user.id);
                }
                catch (error) {
                ...
                await insertStmt.run(guildId, user.id, sanitizedPlayerName, points, wins, draws, bonus);
                await interaction.reply({ content: `Os dados do <@!${user.id}> foram adicionados com sucesso!`, ephemeral: true });
